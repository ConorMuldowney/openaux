...
import { POST } from "@/app/api/scoring/ranked-ballot/route";

function makeRequest(body: unknown): Request {
  return new Request("http://localhost/api/scoring/ranked-ballot", {
    method: "POST",
...
describe("POST /api/scoring/ranked-ballot contract", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns validation envelope for invalid JSON", async () => {
    const response = await POST(
      new Request("http://localhost/api/scoring/ranked-ballot", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: "not-json",
      }),
